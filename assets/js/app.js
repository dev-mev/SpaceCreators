...
$("#newPlanetForm").submit((event) => {
  ...
  const newDiv = $(`<div
    data-ptr-autospin="${newPlanetObj.speed}ms"
    data-ptr-angle="${newPlanetObj.angle}deg"
    data-ptr-glow="0 0 50px rgba(236,206,20,0.35), inset 33px 20px 50px rgba(0,0,0,0.5)"
    data-ptr-size="${newPlanetObj.size}"
    data-ptr-pattern="${newPlanetObj.img}"
    data-ptr-ringcolor="${newPlanetObj.ringColor}"
    data-ptr-ringangle="${newPlanetObj.ringAngle}deg"
    data-ptr-ring="true"
    class="planet">
    </div>`);
...
$("#random-planet-button").on("click", function () {
  ...
  const angle = generateRandomNumber(0, 360);
  const newDiv = $(`<div
        data-ptr-autospin="${generateRandomNumber(2000, 6000)}ms"
        data-ptr-angle="${angle}deg"
        data-ptr-glow="0 0 50px rgba(236,206,20,0.35), inset 33px 20px 50px rgba(0,0,0,0.5)"
        data-ptr-size="${planetSize}x${planetSize}"
        data-ptr-pattern="https://source.unsplash.com/random?sig=${generateRandomNumber(0, 1000)}"
        data-ptr-ringcolor="#${(Math.random() * 0xFFFFFF << 0).toString(16)}"
        data-ptr-ringangle="${angle}deg"
